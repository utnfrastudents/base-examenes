...
build: $(PROJECT)
	...
	&& ./$(DEBUGDIR)$^
$(PROJECT): $(OBJDIR)$(STRO) $(OBJDIR)$(ARRO) $(OBJDIR)$(INPO) $(OBJDIR)$(MENO) $(OBJDIR)$(ENTO) $(OBJDIR)$(PERO) $(OBJDIR)$(OBJO) $(OBJDIR)$(OBJ)
...
$(OBJDIR)$(OBJO): $(OBJC)
	@cd $(OBJDIR) \
	&& $(CC) $(CFLAGS) $(UPDIR)$^ \
	&& cd $(UPDIR) && echo "Se compila a codigo objeto objects.c" $(LOG)
$(OBJDIR)$(PERO): $(PERC)
	@cd $(OBJDIR) \
	&& $(CC) $(CFLAGS) $(UPDIR)$^ \
